...
import type { AiRecommendation, Bilingual, MarketSnapshot } from "@/lib/types";
import {
  computeATR,
  computeMarketScore,
...
import { getNewsRisk } from "@/lib/mockNews";
import {
  geminiEnabled,
  generateNewsImpact,
...
// Builds the full snapshot from real OHLC inputs plus derived analytics.
// The AI fields start as the rule-based stub; applyAi() upgrades them to Gemini.
...
// Deterministic fallback so the dashboard never crashes if the feed is down.
...
// The deterministic snapshot refreshes every poll (~10s); the (slower, paid)
// AI fields refresh at most once per TTL. Stale AI is served immediately while
// a background refresh runs, so users never wait on Gemini after the first load.
// ---------------------------------------------------------------------------
const AI_TTL_MS = 300_000; // 5 minutes — conserves the Gemini free-tier quota
...
// Calls Gemini and refreshes the cache. On any failure, caches the stub so we
// don't hammer the API; it will retry after the TTL expires.
async function refreshAi(s: MarketSnapshot): Promise<void> {
  ...
  try {
    ...
    aiCache = {
      recommendation: s.recommendation, // already the stub
      newsImpact: null,
      newsKey: key,
      at: Date.now(),
      source: "fallback",
...
// Applies cached/fresh Gemini output onto the snapshot.
async function applyAi(s: MarketSnapshot): Promise<void> {
  if (!geminiEnabled()) return; // leave the stub in place
  ...
  if (!aiCache) {
    ...
    // Stale: serve current cache now, refresh in the background.
...
export async function GET() {
  ...
  try {
    ...
    // For a multi-day range, meta.chartPreviousClose is the close *before the
    // whole window* (a month ago), not yesterday. Use the prior daily candle's
    // close as the true previous close; fall back to meta only if unavailable.
    const previousClose =
  ...
  // Upgrade the AI fields with Gemini (no-op if no key / on error).
